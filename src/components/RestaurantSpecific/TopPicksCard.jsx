...
const TopPicksCard = memo(({ data, restaurantData }) => {
  ...
  const price = finalPrice ? (
    <p className="flex items-center gap-1 font-semibold tracking-tight text-white">
      <span className="text-gray-300 line-through">₹{defaultPrice} </span>₹
      {finalPrice}{' '}
      <svg
        className="inline"
        width="16"
    ...
    <p className="font-semibold tracking-tight text-white">₹{defaultPrice}</p>
  );
  ...
  return (
    <div
      className={`relative my-2 mb-8 flex h-72 w-72 shrink-0 flex-col justify-between p-4`}
    >
      <img
        src={isError ? '/images/fallback.png' : imageUrl}
        alt={data?.name}
        className="absolute top-0 left-0 -z-10 h-full w-full rounded-2xl object-cover"
      ...
      <div
        id="button"
        className="mt-auto flex items-center justify-between pl-0.5"
      >
        <div
        ...
        >
          ...
          <div
            ...
            onClick={() =>
              wishlistAddHandler(
                { restaurantData, item: data, quantity: 1 },
                data?.id,
              )
            }
            style={{
        ...
        <AddToCartBtn
          data={{ restaurantData, item: data, quantity: 1 }}
          pX="px-7"
        />
      </div>
    </div>
