    ...
    createDebounce((searchTerm) => {
      ...
      const FilteredData = AllItems.filter((obj) => {
        return obj?.name
          .toLowerCase()
          .split(' ')
          .some((word) => word.startsWith(searchTerm.trim().toLowerCase()));
      });
        ...
        )}
        {
          searchTerm.trim() !== '' ? (
            searchData === null ? (
              ...
              <p className="my-5 text-center font-semibold text-gray-700">{`Sorry, we couldn't find any items matching your search.`}</p>
            )
          ) : null
