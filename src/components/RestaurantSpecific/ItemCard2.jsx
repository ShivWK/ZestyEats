...
const ItemCard2 = memo(
  ({ item, isParentOpen, restaurantData = null, opened }) => {
    ...
    const price = finalPrice ? (
      <p className="flex items-center gap-1 text-sm font-semibold tracking-tight text-gray-800 dark:text-white">
    ...
    return (
      <div className="flex w-full flex-col justify-between overflow-hidden border-b-[1px] border-gray-300 bg-white p-1 pt-2 md:flex-row md:px-1 md:py-4 md:pt-0 dark:bg-gray-800">
        <div className="dark:bg-gray-00 order-2 mt-3 flex max-w-[525px] flex-col items-start gap-1.5 p-2 md:order-1">
          {veg ? (
            <svg
              width="15"
              height="15"
          ...
          <p className="text-lg font-bold text-gray-800 dark:text-white">
            {textToZestyEats(item?.name)}
          </p>
          <div>{price}</div>
          {ratingObject.rating && (
            <div className="flex items-center gap-1">
              <i className="ri-star-fill text-green-700 dark:text-green-400"></i>{' '}
              <p className="mt-0.5 flex items-center gap-0.5 text-sm font-bold text-green-700 dark:text-white">
                {ratingObject?.rating}
          ...
          <div
            ref={containerRef}
            className="relative overflow-hidden transition-all duration-100 ease-linear"
            style={{
          ...
          >
            <p
              ref={paraRef}
              className="text-sm text-wrap text-black dark:text-gray-200"
            >
              {textToZestyEats(item?.description)}
            </p>
          </div>
          {overFlow && !isOpen && (
            <span
              className="dark:text-primary -mt-4 inline-flex w-fit cursor-pointer items-center gap-2 font-bold text-gray-600"
              onClick={() => setIsOpen(!isOpen)}
            >
              ...more
              <i
                className={`ri-arrow-down-s-fill mt-1 -ml-2.5 inline transform text-2xl font-[200] text-[#ff5200] transition-transform duration-100 ease-in`}
              ></i>
            </span>
          )}
        </div>

        <div className="relative order-1 mt-4 h-40 w-full shrink-0 overflow-hidden rounded-xl md:order-2 md:h-48 md:w-48">
          <img
          ...
          <div className="absolute top-[75%] -translate-x-1/2 transform max-lg:-right-9 md:top-[80%] md:left-1/2">
          ...
          <div
            ...
            onClick={() =>
              wishlistAddHandler(
                { restaurantData, item, quantity: 1 },
                item?.id,
              )
            }
            style={{
