...
const UserAddress = ({
  ...
  return (
    <>
      <div
        className={`shrink-0 self-start rounded-xl bg-gray-100 p-2 ${width} border-primary mx-auto border lg:w-[55%] dark:bg-gray-300`}
      >
        ...
        <div className="mt-2 ml-auto flex items-center gap-2 text-sm">
          <button
            onClick={() => {
              dispatch(setHideEditAddressModal(false));
              dispatch(setEditAddressModal(true));
            }}
            className="bg-primary dark:bg-darkPrimary rounded px-3 py-0.5 font-medium tracking-wide text-white transition-all duration-75 ease-linear active:scale-95"
          >
            Edit
          </button>
          {pathName === '/paymentsAndAddresses' &&
            (isDeliverableCompo ? (
              deliverAt?.latLong?.lat === address?.latLong?.lat &&
              deliverAt?.latLong?.lng === address?.latLong?.lng ? (
              ...
              <div className="flex items-center gap-1">
                ...
                />
                <span className="font-sans text-sm font-semibold tracking-wider text-red-500">
                  Not Deliverable
                </span>
              </div>
            ))}
          ...
          {pathName === '/mobileProfileResponse' && (
            <button
              ...
              className="bg-primary dark:bg-darkPrimary flex h-6 w-18 items-center justify-center rounded font-medium tracking-wide text-white transition-all duration-75 ease-linear active:scale-95"
            >
      ...
      {editAddressModal && !addAddressModal && (
        <div
          ...
          className="fixed top-0 left-0 z-50 h-full w-full bg-black/60"
        >
