...
const RestaurantImageCard = ({
  ...
  useEffect(() => {
    const exist = favoriteRestaurants?.find(
      (obj) => obj.data?.id === dataToMap?.id,
    );
  ...
  return (
    <div
      className={`relative flex h-40 w-full shrink-0 items-end rounded-2xl bg-cover bg-center bg-no-repeat p-3 max-md:max-h-full max-md:min-h-52 max-md:basis-[48%] md:pl-4 ${!disable && 'max-md:shadow-[0_0_10px_2px_rgba(0,0,0,0.5)] dark:shadow-[0_0_10px_2px_rgba(156,145,145,0.5)]'} ${
        from === 'online'
          ? 'md:h-[160px]'
          : from === 'specificFood'
            ? 'md:h-[240px]'
    ...
    >
      ...
      <div
        className="absolute top-2.5 right-2.5 flex cursor-pointer items-center justify-center rounded-full p-0.5"
