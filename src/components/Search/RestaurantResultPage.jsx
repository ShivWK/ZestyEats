...
import { NavLink, Await, useLoaderData, useSearchParams } from "react-router-dom";
import { Suspense } from "react";

const MainContent = ({ data, lat, lng }) => {
  ...
  return (
    <div className="p-3 pt-8 w-full bg-gray-100 flex flex-col">
      ...
      <div className="flex flex-wrap gap-1 gap-y-3 justify-between">
        {moreRestros && moreRestros.map((data) => <Card key={data?.id} data={data} />)}
      </div>
    </div>
  );
};
...
const RestroData = () => {
    ...
    return <Suspense fallback={<h1>Loading...</h1>}>
        <Await resolve={data}>
            {data => <MainContent data={data} lat={lat} lng={lng} />}
        </Await>
    </Suspense>
}
