...
const Card = ({ data, lat, lng }) => {
  ...
  const defaultPrice =
    dishData?.price / 100 || dishData?.defaultPrice / 100 || 0;
  const finalPrice = dishData?.finalPrice / 100;
  const price = finalPrice ? (
    <p className="flex items-center gap-1 text-sm font-bold tracking-tight">
      <span className="text-gray-500 line-through dark:text-gray-700">
        ₹{defaultPrice}{' '}
      </span>
      ₹{finalPrice}
    </p>
  ) : (
    <p className="text-sm font-bold tracking-tight">₹{defaultPrice}</p>
  );
  ...
  return (
    <div className="rounded-md bg-white pb-1 max-md:basis-full md:basis-[49%] dark:bg-gray-300">
      <Link
        to={path}
      ...
      >
        <div className="basis-[95%]">
          <p className="line-clamp-2 leading-5 font-bold break-words text-gray-800 md:w-full dark:text-black">
            {restaurantData?.name}
          </p>
        ...
        <i className="ri-arrow-right-long-fill basis-[3%] transform cursor-pointer text-2xl text-gray-600 transition-all duration-150 ease-in-out group-hover:translate-x-[6px] md:basis-[5%] dark:text-black"></i>
      </Link>
      <hr className="my-1.5 text-gray-300 dark:text-black" />
      <div className="flex justify-between">
        <div className="flex flex-col gap-2 p-4">
          ...
          <div className="w-[100%]">
            <p className="line-clamp-2 leading-5 font-bold tracking-tight">
              {dishData?.name}
            </p>
          </div>
          <div>{price}</div>
          {dishData?.description && (
            <div
              ...
              id="moreCities"
              className="linear mt-1 flex w-fit cursor-pointer items-center gap-2.5 rounded-2xl border-[1px] border-gray-500 px-2 py-0.5 transition-all duration-300 hover:bg-gray-100 md:border-2 dark:border-gray-700 dark:text-gray-700"
            >
              <p className="text-sm font-normal text-gray-900 select-none">
                More Details
              </p>
              <i
                className="fa-solid fa-caret-down linear text-gray-900 transition-all duration-300"
                style={{
        ...
        <div className="relative m-2 h-36 w-36 shrink-0 overflow-hidden rounded-xl">
          ...
          <div className="absolute top-[75%] left-1/2 -translate-x-1/2 transform">
            <AddToCartBtn
              data={{
                restaurantData: restaurantDataToWishlist,
                item: dishData,
                quantity: 1,
              }}
            />
          ...
          <div
            className="absolute top-2.5 right-2.5 flex cursor-pointer items-center justify-center rounded-[9999px] p-0.5"
            onClick={() =>
              wishlistAddHandler(
                {
                  restaurantData: restaurantDataToWishlist,
          ...
          >
            <svg
              ...
              viewBox="0 0 24 24"
              fill="white"
              className="wishlist-icon h-6 w-6"
            >
      ...
      <div
        className="overflow-hidden bg-white px-2 font-medium text-gray-700 transition-all duration-100 ease-linear dark:bg-gray-300 dark:text-black"
...
const MainContent = ({ data, lat, lng, mode }) => {
  ...
  return (
    <div className="mt-14 rounded-md bg-gray-200 p-1.5 dark:bg-gray-800">
      <div className="-mt-1.5 mb-2.5 w-full">
        <Filter text1="Veg" text2="Non Veg" />
      </div>
      <div className="flex w-full flex-wrap items-start justify-between gap-2.5 gap-y-4">
        {cards ? (
          cards.map((item) => {
          ...
          <p className="self-center pt-2 pb-4 text-center font-semibold">
