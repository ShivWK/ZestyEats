    ...
    const resendClickHandler = async (e) => {
      ...
      try {
        ...
        const res = await fetch(
          `${import.meta.env.VITE_BASE_URL}/api/user/resendOtp/${mode}`,
          {
            method: 'POST',
            headers: {
    ...
    return (
      <>
        <div
          ...
          className="relative h-[70px] cursor-text border-2 border-gray-300 p-2.5"
        >
          <p
            className={`absolute font-semibold ${
              hasValue ? 'text-red-500' : 'text-gray-400 dark:text-gray-300'
            } transform transition-all duration-150 ease-linear ${
              isEntryMade
                ? 'top-2.5 text-xs'
          ...
          <input
            ...
            className={`relative top-5 w-full bg-transparent text-lg font-semibold outline-none dark:text-white`}
          />
        </div>
        {!fullDisable
          ? purpose === 'otp' && (
              <div className="mt-1 flex justify-between px-0.5">
                <button
                  ...
                  disabled={disable}
                  className={`text-xs font-bold ${disable ? 'text-gray-400' : 'text-primary cursor-pointer'} underline underline-offset-2`}
                >
                  Resend OTP
                </button>
                {disable && (
