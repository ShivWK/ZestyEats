  ...
  async function sendOTP(data, token, otpOnPhoneStatus) {
    ...
    try {
      const resp = await fetch(
        `${import.meta.env.VITE_BASE_URL}/api/user/login/sendOtp/${mode}`,
        {
          method: 'POST',
          headers: {
    ...
    if (data.get('otp').length === 0) {
      ...
      try {
        const res = await fetch(
          `${import.meta.env.VITE_BASE_URL}/api/user/verifyOtp/${mode}/login`,
          {
            method: 'POST',
            headers: {
              'Content-Type': 'application/json',
    ...
    <>
      {!isOtpSend && (
        <div
          ...
          className={`relative flex h-8 w-16 items-center justify-between overflow-hidden rounded-full border-2 border-gray-200 ${!isOtpSend && 'cursor-pointer'} bg-gray-200`}
        >
          <Phone size={18} strokeWidth={2} className="ml-1.5 text-green-500" />
      ...
      <div className="mt-5">
        {!isOtpSend &&
          (otpOnPhone ? (
            <p className="mt-2 text-xs font-bold tracking-wide text-gray-600 dark:text-gray-300">
              OTP will be sent to{' '}
