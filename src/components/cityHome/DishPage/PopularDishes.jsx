...
const MainData = ({ data }) => {
    ...
    if (mainData?.data) {
        ...
        return <>
        <main className="w-full md:max-w-[1070px] mx-auto overflow-hidden pt-[5rem] md:pt-28 overflow-x-hidden max-md:px-2 flex flex-col gap-2.5 md:gap-5 min-h-96">
                    ...
                    <section className="flex flex-col gap-0.5 md:gap-1 dark:bg-gray-800 bg-gray-200 rounded p-1 pt-3 md:p-7 mt-2 md:mb-3">
                        ...
                        {currentDataSet.length !== mainData.data.length && (
                            <button onClick={loadMoreClickHandler} className="px-4 py-1 mb-3 md:mb-1 border-[1px] border-primary rounded bg-primary text-white font-semibold w-fit self-center cursor-pointer active:scale-95 active:bg-gray-300 active:border-gray-500 active:text-black transition-all duration-100 ease-linear ">Load more</button>
                        )}
                    </section>
