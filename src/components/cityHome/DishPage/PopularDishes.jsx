...
const MainData = ({ data }) => {
  ...
  if (mainData?.data) {
    ...
    const [currentDataSet, setCurrentDataSet] = useState(
      mainData.data.slice(0, lastLength),
    );

    const loadMoreClickHandler = () => {
      setCurrentDataSet((prv) => [
        ...prv,
        ...mainData.data.slice(lastLength, lastLength + 10),
      ]);
    ...
    return (
      <>
        <main className="mx-auto flex min-h-96 w-full flex-col gap-2.5 overflow-hidden overflow-x-hidden pt-[5rem] max-md:px-2 md:max-w-[1070px] md:gap-5 md:pt-28">
          <section className="self-start">
            <BreadcrumbsWrapper />
          </section>
          <h1 className="text-xl max-md:leading-6 md:text-3xl dark:text-white">
            {mainData.heading}
          </h1>
          <section className="mt-2 flex flex-col gap-0.5 rounded bg-gray-200 p-1 pt-3 md:mb-3 md:gap-1 md:p-7 dark:bg-gray-800">
            <h2 className="ml-1.5 text-[1.10rem] font-bold text-gray-800 md:text-xl dark:text-gray-300">{`${mainData.data.length} dishes found for ${dish}`}</h2>
            <div className="">
              {currentDataSet.map((obj) => (
                <RestaurantCart
                  key={obj.restaurant.info.id}
                  data={obj}
                  latLng={mainData.latLng}
                />
              ))}
            </div>
            {currentDataSet.length !== mainData.data.length && (
              <button
                onClick={loadMoreClickHandler}
                className="border-primary bg-primary mb-3 w-fit cursor-pointer self-center rounded border-[1px] px-4 py-1 font-semibold text-white transition-all duration-100 ease-linear active:scale-95 active:border-gray-500 active:bg-gray-300 active:text-black md:mb-1"
              >
                Load more
              </button>
            )}
    ...
    return (
      <main className="mx-auto flex min-h-96 w-full flex-col items-center gap-2.5 overflow-hidden overflow-x-hidden pt-[5rem] pb-16 max-md:px-2 md:max-w-[1070px] md:pt-28 md:pb-18 dark:text-gray-300">
        <svg
          xmlns="http://www.w3.org/2000/svg"
          className="mt-4 h-[250px] w-[300px] lg:mt-4 lg:h-[300px] lg:w-[350px]"
          width="888"
          height="485.489"
        ...
        <p className="text-center text-xl font-semibold text-black lg:text-3xl dark:text-white">
          We couldn’t find any items here.
        </p>
        <p className="text-center font-medium text-gray-500 lg:text-xl dark:text-gray-300">
