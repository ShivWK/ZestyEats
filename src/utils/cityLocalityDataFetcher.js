const cityLocalityDataFetcher = (cards) => {
  ...
  const topChainRestaurantData =
    cards?.find((item) => item?.card?.card?.id === 'top_brands_for_you')?.card
      ?.card?.gridElements?.infoWithStyle?.restaurants || [];
  const topChainRestaurantTitle = cards?.find(
  ...
  const cuisineData =
    cards?.find((item) => item?.card?.card?.id === 'cuisines_near_you')?.card
      ?.card?.cuisines || [];
  const cuisineTitle = cards?.find(
  ...
  return {
    ...
    foodieData,
    restaurantChain: {
      data: topChainRestaurantData,
      title: topChainRestaurantTitle,
    },
    onlineRestaurant: {
